...
int timer_init(void) {
	...
	if (result < 0) {
	printk(
	  "<1>timer: cannot obtain major number %d\n", timer_major);
	return result;
	}
/*Reserve timer memory region */
	if(request_mem_region(TIMER_BASE, RANGE, DEVICE_NAME) == NULL) //_mem_
	{
		printk(
	  		"<1>timer: cannot reserve memory \n");
...
ssize_t timer_read(struct file *filp, char *buffer, size_t length, loff_t * offset){
    ...
    *offset+=1; 
	...
	/*Convert to ASCII */
	sprintf(msg, "%02hhx%02hhx%02hhx%02hhx", msg[3],msg[2],msg[1], msg[0]);
	msg[8] = '\n';
